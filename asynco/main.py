...
class Promise(Generic[T]):
    def __init__(self) -> None:
        self._state: Union[T, PendingType] = Pending

    @property
    def value(self) -> T:
        if isinstance(self._state, PendingType):
            raise PendingException("The return value of this task is still pending.")
        return self._state

    @property
    def ready(self) -> bool:
        return not isinstance(self._state, PendingType)


class TaskGroup(_TaskGroup):
    def run(self,
                async_function: Callable[T_ParamSpec, Awaitable[T]],
                name: object = None) -> Callable[T_ParamSpec, Promise[T]]:
        @functools.wraps(async_function)
        def wrapper(*args: T_ParamSpec.args, **kwargs: T_ParamSpec.kwargs) -> Promise[T]:
...
def asyncify(function: Callable[T_ParamSpec, T_Retval],
             *,
             cancellable: bool = False,
             limiter: Optional[anyio.CapacityLimiter] = None) -> Callable[T_ParamSpec, Awaitable[T_Retval]]:
